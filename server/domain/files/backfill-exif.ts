...
import type { FileRow } from '../../utils/db'
...
import { db, files as filesTable } from '../../utils/db'
...
async function readExif(buffer: Buffer): Promise<ExifResult | null> {
  try {
    ...
    logger.warn('Failed to read EXIF:', { error })
    return null
  }
}
...
export interface ExifBackfillSummary {
  total: number
  updated: number
  skipped: number
}

export async function backfillExifMetadata(): Promise<ExifBackfillSummary> {
  ...
  for (const file of rows) {
    ...
    if (!sourceUrl) {
      logger.warn(`File #${file.id} has no image url; skip`)
      skipped += 1
      continue
    }
  ...
  logger.info(`Backfill exif done. Updated ${updated}, skipped ${skipped}.`)
