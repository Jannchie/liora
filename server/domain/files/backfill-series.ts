...
import { slugifySeriesTitle } from '../series/service'
...
export async function backfillSeries(): Promise<SeriesBackfillSummary> {
  ...
  if (candidates.length === 0) {
    logger.info('[backfill-series] no files with fanworkTitle found, nothing to do.')
    return { totalFiles: 0, createdSeries: 0, linkedFiles: 0, totalSeries: 0 }
  }
  ...
  for (const [title, filesInGroup] of grouped.entries()) {
    ...
    if (!targetSeries) {
      ...
      logger.info(`[backfill-series] created series "${title}" (${uniqueSlug})`)
    }
    ...
    if (newEntries.length > 0) {
      ...
      linkedFiles += newEntries.length

      logger.info(`[backfill-series] linked ${newEntries.length} files to "${title}"`)
    }
  ...
  logger.info(`[backfill-series] completed. total series: ${finalSeries.length}`)
  return {
