...
async function requestPresignedUploads(imageFile: File, videoFile: File | null): Promise<PresignResponse> {
...
function updateProgress(loaded: number, offset: number, total: number): void {
  uploadBytesSent.value = Math.min(total, offset + loaded)
  uploadProgress.value = total > 0 ? Math.min(100, (uploadBytesSent.value / total) * 100) : 0
  const startedAt = uploadStartedAt.value
  if (startedAt !== null) {
    const elapsedSeconds = (performance.now() - startedAt) / 1000
    if (elapsedSeconds > 0) {
      uploadSpeed.value = uploadBytesSent.value / elapsedSeconds
    }
  }
}

function uploadFileToPresignedUrl(file: File, presign: PresignedUpload, offset: number, total: number): Promise<void> {
  return new Promise((resolve, reject) => {
    const xhr = new XMLHttpRequest()
    xhr.upload.addEventListener('progress', (event) => {
      if (event.lengthComputable) {
        updateProgress(event.loaded, offset, total)
      }
    })
    ...
    xhr.addEventListener('load', () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        updateProgress(file.size, offset, total)
        resolve()
      }
      else {
...
async function sendFileWithProgress(imageFile: File, videoFile: File | null): Promise<{ uploadId?: string }> {
  ...
  const presign = await requestPresignedUploads(imageFile, videoFile)

  let offset = 0
  await uploadFileToPresignedUrl(imageFile, presign.image, offset, totalBytes)
  offset += imageFile.size
  if (videoFile && presign.video) {
    await uploadFileToPresignedUrl(videoFile, presign.video, offset, totalBytes)
    offset += videoFile.size
  }

  updateProgress(totalBytes, 0, totalBytes)
  const response = await requestFetch<{ uploadId?: string }>('/api/files', {
    ...
    body: {
      ...
      notes: form.notes,
      livePhotoStillTime: isLiveMode.value ? liveFrameTime.value : undefined,
    },
  })
...
async function submit(): Promise<void> {
  ...
  stopProcessingStatusPoll()
  try {
    const { uploadId } = await sendFileWithProgress(selectedFile.value, selectedVideo.value)
    clearSelectedFile()
    if (uploadId) {
